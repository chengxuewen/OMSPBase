...
- libwebrtc (via webrtc-sys): 完整 GCC + FEC + NetEQ，公网弱网/遥控场景
...
- Phase 1: 单一 libwebrtc 后端。str0m/libwebrtc/webrtc-rs 三后端为 Phase 2+ 架构
## D12: 信令服务混合架构
...
**日期**: 2026-07-16
**原因**:
- backend-str0m (默认, Embed/LAN), backend-libwebrtc (弱网/遥控), backend-webrtc-rs (未来)
...
## D66: omspbase-remote-sdk — 远程收图+控制 SDK
...
**决策**: Rust crate 命名 omspbase-field / omspbase-remote，C/C++ FFI 加 -c 后缀（omspbase-field-c / omspbase-remote-c）
**日期**: 2026-07-17
**原因**:
...
**决策**: omspbase-remote 和 omspbase-field 作为 facade crate，各自 re-export 子 crate 类型（core/codec/transport/signaling/pipeline），舱端只需一 dep
**日期**: 2026-07-17
**原因**:
- 舱端 app 只需 `omspbase-remote = "0.1"` 一行依赖
...
## D70: omspbase-remote-c 静态链接 FFmpeg

**决策**: 舱端 C 库 (omspbase-remote-c) 静态链接 FFmpeg (.a)，支持 HW decode，field 端保持 GStreamer
**日期**: 2026-07-17
**原因**:
...
## D76: omspbase-remote 与 omspbase-client 分离

**决策**: omspbase-remote(-c) 为无 GUI SDK，omspbase-client 为带 Tauri GUI 的独立二进制。omspbase-client 可直接依赖底层 Rust crate
**日期**: 2026-07-17
**原因**:
- omspbase-remote-c: .a 静态库，供 C/C++ 嵌入（ROS/自驾/移动端）
...
**决策**: omspbase-field 和 omspbase-remote 提供 C (.h+.a)、C++ (header-only RAII wrapper)、Python (pyo3 native) 三套绑定
**日期**: 2026-07-17
**原因**:
- C: 最底层 FFI，所有语言可调用
...
**决策**: omspbase-field 和 omspbase-remote 同时提供静态库 (.a) 和动态库 (.so/.dylib/.dll)
**日期**: 2026-07-17
**原因**:
- 静态链接 .a：嵌入车载控制器等资源受限环境，无运行时依赖，编译器可内联优化
- 动态链接 .so：用于插件系统（dlopen）和多进程共享（减少内存占用）
- C FFI 接口相同（omspbase-field-c / omspbase-remote-c），链接方式不影响 API
...
| 7 | omspbase-remote | facade | core, transport, signaling | ~150 |
...
**决策**: Phase 1 GitHub Actions + single-platform build
**日期**: 2026-07-17
**原因**:
- 构建矩阵: Ubuntu x86_64 + ARM64 (Jetson) + macOS + Windows
- 跨编译: cross + qemu 模拟
- 产物: crates.io + GitHub Releases + Docker Hub
- 阶段: lint → test → build → package → sign
...
- omspbase-remote-sdk-v0.1.0-x86_64.tar.gz (~5 MB, FFmpeg)
- omspbase-remote-sdk-v0.1.0-aarch64.tar.gz (~5 MB)
...
| 平台 | CPU | RAM | GPU | 备注 |
|------|-----|-----|-----|------|
| macOS | Apple M1+ | 8GB | Integrated (VideoToolbox) | macOS 14+ |
...
**新增产物**: crates/omspbase-server, crates/omspbase-remote, config/server.conf, config/remote.conf, docs/modules/13-server-architecture.md, docs/modules/14-remote-architecture.md
...
**计划概要**：
- 32 任务，~3300 行 Rust
- 4 个 workspace crate：omspbase-core + omspbase-host + omspbase-server + omspbase-remote
- 7 篇 SDD 覆盖核心模块已有
- 测试 ≥80% 覆盖率，4 层策略
...
|------|-----|-----|-----|------------|
| macOS | Apple Silicon M1+ or Intel i5+ | 8GB | 集成 GPU (VideoToolbox) | macOS 13+ |
