...
# Copy binary
cp omspbase-host "$PREFIX/bin/"
chmod +x "$PREFIX/bin/omspbase-host"
...
if command -v systemctl &> /dev/null; then
    cp omspbase-host.service /etc/systemd/system/
    systemctl daemon-reload
    systemctl enable omspbase-host
    echo "systemd service registered. Edit $PREFIX/etc/host.conf then:"
    echo "  systemctl start omspbase-host"
else
    echo "No systemd found. Start manually:"
    echo "  $PREFIX/bin/omspbase-host --config $PREFIX/etc/host.conf"
fi
