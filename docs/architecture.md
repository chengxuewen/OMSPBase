...
> 🔄 **D126-D155 增量 (2026-07-19)**: 三层逻辑抽象模型 (D126)、Component 架构 (D127-D134)、WebRTC 架构升级为 webrtc-sys RTP track (D137)、mediasoup SFU (D138)、统一 Gateway (D128)、多后端 feature gate (D139-D140, webrtc-sys/webrtc-rs/stub, str0m Phase 2+)、webrtc-kit 设计借鉴 (D144-D151)、Host 单体架构确认 (D155)。实施详见 [整合实施计划](../.sisyphus/plans/consolidated-mvp/plan.md)。
...
| **Process 层** | OS 进程 (systemd/K8s) | hostd, omspbase-server, omspbase-client |
