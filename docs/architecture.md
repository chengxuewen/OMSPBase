...
> Phase 0 — 架构定义 | 2026-07-16 | 最后同步 decisions.md: D118 (2026-07-17)
...
| 能力 | Phase | 说明 | 典型场景 |
|------|-------|------|---------|
| 远程桌面 | Phase 1 | 屏幕捕获、GPU 编码、输入注入 | IT 运维、远程办公 |
| 视频会议 | Phase 2+ | 多方音视频、SFU/MCU、屏幕共享 | 团队协作 |
| 推拉流 | Phase 1 | RTMP/HLS/SRT 接入与分发 | 直播、内容分发 |
| 监控相机 | Phase 2+ | ONVIF/GB28181 发现与流管理 | 安防、巡检 |
| WebRTC 遥操作 | Phase 1 | 低延迟视频 + DataChannel 控制 | 车辆遥控、机器人 |
| 车端推流 | Phase 1 | 车辆摄像头推流到云端 | 车联网 |
| 舱内拉流 | Phase 1 | 舱内屏幕观看远程视频 | 远程驾驶 |
...
当前 MVP workspace 含 3 个 Cargo member crate（D118 确认）。Phase 2+ 逐步扩展 transport、signaling、codec、pipeline 等领域 crate。视频会议相关 crate（omspbase-conference 等）为 Phase 2+ 规划。

```
crates/
├── omspbase-host/          Host 应用 (headless, 采集+编码+推流+信令+配置)
├── omspbase-remote/        Remote 应用 (拉流+解码+渲染+控制)
└── omspbase-server/        Server 应用 (信令 relay+监控+会话管理)
  
Phase 2+ 计划 crates: omspbase-core, omspbase-transport, omspbase-signaling, omspbase-codec, omspbase-pipeline, omspbase-auth, omspbase-field, omspbase-field-c, omspbase-remote-c, omspbase-napi
```
...
| **WebRTC 栈** | str0m (Embed/LAN) + libwebrtc (弱网/遥操) + webrtc-rs (future) | 三后端编译期分发 (D11, D32) |
...
| 3 | Cargo workspace 组织结构 — 3 crates (Phase 1) → 10+ crates (Phase 2+) | ✅ D16, D82 |
