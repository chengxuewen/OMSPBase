...
**Relay Engine 核心原则** (Phase 1): Server 不解码也不重编码视频帧。Relay 引擎在 RTP 层面做 track bridging，两条 PeerConnection 之间直接转发 RTP packet。延迟增量 ≤30ms。

### 13.2.1 mediasoup SFU 集成 (Phase 2)

Phase 2 引入 mediasoup SFU 替代 Phase 1 的同进程 PeerConnection bridge，提升多参与方场景的媒体转发效率。
